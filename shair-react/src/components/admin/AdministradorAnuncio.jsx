...
function AdministradorAnuncio() {
  ...
  const [
    editarSeleccionAnuncioInterruptor,
    setEditarSeleccionAnuncioInterruptor,
  ] = useState(false);
  ...
  useEffect(() => {
    const leerAnuncio = async () => {
      try {
        ...
        const anunciosFiltrados = anunciosTotales.filter(
          (anuncio) => anuncio.id_administrador === idAdministradoresAnuncios
        );

        setAnuncios(anunciosFiltrados);

        const nombres = anunciosFiltrados.map((anuncio) => anuncio.nombre);
        const enunciados = anunciosFiltrados.map(
          (anuncio) => anuncio.enunciado
        );
        const imagenes = anunciosFiltrados.map(
          (anuncio) => anuncio.archivo_adjunto
        );
        const fechasExpiracion = anunciosFiltrados.map(
          (anuncio) => anuncio.fecha_expiracion
        );
        const administradores = anunciosFiltrados.map(
          (anuncio) => anuncio.Administrador.nombre
        );
        const masInformaciones = anunciosFiltrados.map(
          (anuncio) => anuncio.mas_informacion
        );
  ...
  useEffect(() => {
    const deseleccionarAnuncio = (event) => {
      if (
        seleccionadoAnuncio.current &&
        !seleccionadoAnuncio.current.contains(event.target)
      ) {
  ...
  const cancelarEditarAnuncio = (id) => {
    ...
    setEditarSeleccionAnuncio((prevState) =>
      prevState.filter((idNoticia) => idNoticia !== id)
    );
    setEditarSeleccionAnuncioInterruptor(
      () => !editarSeleccionAnuncioInterruptor
    );
  ...
  const actualizarAnuncio = async (id) => {
    ...
    try {
      ...
      setEditarSeleccionAnuncio((prevState) =>
        prevState.filter((idAnuncio) => idAnuncio !== id)
      );
    } catch (error) {
  ...
  const actualizardatos = (id) => {
    ...
    setNombresAnuncios((prevState) =>
      prevState.filter((_, index) => index !== anuncioIndex)
    );
    setEnunciadosAnuncios((prevState) =>
      prevState.filter((_, index) => index !== anuncioIndex)
    );
          ...
          <>
            {anuncios.map((anuncio, index) => (
              <div
                ...
                onClick={() => seleccionarAnuncio(anuncio.id)}
                ref={
                  seleccionAnuncio === anuncio.id ? seleccionadoAnuncio : null
                }
              >
                <form
                  className={
                    editarSeleccionAnuncio.includes(anuncio.id)
                      ? "articuloAdministracionAlternativaContenido  articuloAlternativoAdministracionAlternativaContenido"
                      : "articuloAdministracionAlternativaContenido"
                  }
                >
                  <div className="articuloAdministracionAlternativaContenido0">
                    <input
                      ...
                      id={
                        editarSeleccionAnuncio.includes(anuncio.id)
                          ? "nombreArticuloAlternativoAdministracionAlternativaContenido"
                          : "nombreArticuloAdministracionAlternativaContenido"
                      }
                    ...
                    <fieldset>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          const nuevasFechasExpiracion = [
                            ...fechasExpiracionAnuncios,
                          ];
                    ...
                    <fieldset>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          const nuevosAdministradores = [
                            ...administradoresAnuncios,
                          ];
                    ...
                    <fieldset>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          const nuevasMasInformacion = [
                            ...masInformacionesAnuncios,
                          ];
                  ...
                  <div className="articuloAdministracionAlternativaContenido1">
                    <fieldset className="ArticuloAdministracionAlternativaContenido">
                      <div className="imagenArticuloAdministracionAlternativaContenido">
                        <img
                          src={`http://${localIP}:5000/${anuncio.archivo_adjunto}`}
                ...
                {seleccionAnuncio === anuncio.id &&
                  (!editarSeleccionAnuncio.includes(anuncio.id) ? (
                    ...
                    >
                      <button
                        type="button"
                        onClick={() => cancelarEditarAnuncio(anuncio.id)}
                        className="botonPieAdministracionAlternativaContenidoSeleccionado"
                      >
                        Cancelar
                      </button>
      ...
      contenido = (
        <>
          ...
          >
            ...
            Aquí podrás ver, modificar, eliminar y crear los anuncios, estos se
            asociarán a tu nombre y se eliminaran al pasar la fecha de
            expiración. <br />A continuación, en la parte superior de la página
            encontraras el acceso a el control de anuncios y al formulario de
            creación.
          </motion.p>
        </>
      );
