...
function SuperadministradorRecomendacion() {
  ...
  const actualizarAnuncio = async (id, estado) => {
    const recomendacionIndex = recomendaciones.findIndex(
    ...
    try {
      ...
      setRecomendaciones((prevState) =>
        prevState.map((rec) =>
          rec.id === id ? { ...rec, aprobacion: true } : rec
        )
      );
    } catch (error) {
      ...
      {subSeccion === 8 ? (
        recomendaciones.length > 0 ? (
                    ...
                    {recomendacion.aprobacion ? (
                      <>
                        <motion.button
                          type="button"
                          onClick={() =>
                            actualizarAnuncio(recomendacion.id, false)
                          }
                      ...
                      <>
                        <motion.button
                          type="button"
                          onClick={() =>
                            actualizarAnuncio(recomendacion.id, true)
                          }
                          className="botonPieAdministracionAlternativaContenidoSeleccionado"
                          initial={{ opacity: 0 }}
                          animate={{ opacity: 1 }}
          ...
          <motion.div className="AdministracionAlternativaContenidoSeleccionadoAlternativo">
            <motion.h1
              ...
              transition={{ duration: 0.25 }}
            >
              Parece que aún no se han enviado recomendaciones o se trata de un
              error.
            </motion.h1>
            <motion.p
        ...
        <>
          ...
          >
            ...
            Aquí podrás ver las recomendaciones que hacen los aprendices y
            aprobarlas o eliminarlas.
