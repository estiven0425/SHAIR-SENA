...
function AdministradorCrearNoticia() {
  ...
  const validacion = () => {
    ...
    if (!enunciado) {
      ...
      errors.enunciado =
        "El campo de nombre no puede ser mayor a 1000 caracteres.";
    }
    if (!lugar) {
  ...
  const crearNoticia = async (e) => {
    ...
    try {
      ...
      if (imagen) {
        ...
        const cargaNoticia = await axios.post(
          `http://${localIP}:5000/cargaNoticia`,
          datosFormulario,
          {
            headers: {
      ...
      await axios.post(`http://${localIP}:5000/noticia`, {
        ...
        enunciado,
        archivo_adjunto:
          rutaCargaNoticia === null ? "uploads/logo.png" : rutaCargaNoticia,
        lugar,
      ...
      if (error.response && error.response.data && error.response.data.error) {
        ...
        setServidorError(
          "Error al crear la noticia. Por favor, inténtelo de nuevo."
        );
      }
    }
  };
      ...
      {enviado === true ? (
        <>
          ...
          >
            ...
            <p>
              Ve a la subsección de noticias para ver, editar y eliminar las
              noticias.
            </p>
            <button
              type="button"
            ...
            <fieldset className="subSeccionFormularioRecomendacionPrincipal">
              ...
              >
                {validacionError.imagen && (
                  <span className="subSeccionFormularioAdministracionCrearError subSeccionFormularioAdministracionCrearErrorAlternativa2 ">
                    {validacionError.imagen}
                  </span>
                )}
                {imagen ? (
