...
function AdministradorNoticia() {
  ...
  const [
    editarSeleccionNoticiaInterruptor,
    setEditarSeleccionNoticiaInterruptor,
  ] = useState(false);
    ...
    const leerNoticia = async () => {
      try {
        ...
        const noticiasFiltradas = noticiasTotales.filter(
          (noticia) => noticia.id_administrador === idAdministradoresNoticias
        );
  ...
  const actualizardatos = (id) => {
    ...
    setNombresNoticias((prevState) =>
      prevState.filter((_, index) => index !== noticiaIndex)
    );
    setEnunciadosNoticias((prevState) =>
      prevState.filter((_, index) => index !== noticiaIndex)
    );
        ...
        noticias.length > 0 ? (
          <>
            {noticias.map((noticia, index) => (
              <div
                ...
                ref={
                  seleccionNoticia === noticia.id ? seleccionadaNoticia : null
                }
              >
                <form
                  className={
                ...
                {seleccionNoticia === noticia.id &&
                  (!editarSeleccionNoticia.includes(noticia.id) ? (
                    ...
                    >
                      <button
                        type="button"
                        onClick={() => cancelarEditarNoticia(noticia.id)}
                        className="botonPieAdministracionAlternativaContenidoSeleccionado"
                      >
                        Cancelar
                      </button>
          ...
          <motion.div className="AdministracionAlternativaContenidoSeleccionadoAlternativo">
            ...
            <motion.p
              ...
              transition={{ duration: 0.25 }}
            >
              En el panel superior puedes acceder a la sección para crear una
              noticia.
            </motion.p>
          </motion.div>
        );
      ...
      contenido = (
        <>
          ...
          <motion.p
            ...
            transition={{ duration: 0.25 }}
          >
            Bienvenido a la sección de noticias, aquí podrás ver, modificar,
            eliminar y crear las noticias, estas se asociarán a tu nombre.{" "}
            <br />A continuación en la parte superior de la página, encontrarás
            el acceso al control de noticias y al formulario de creación.
          </motion.p>
        </>
      );
  }
